...
func main() {
	...
	router.GET("/transacciones/busqueda", getByFilters)
	router.GET("/transacciones/:id", getById)

	router.Run()
}

func getByFilters(c *gin.Context) {
...
func getById(c *gin.Context) {
	...
	if err != nil {
		c.JSON(400, gin.H{
			"error": fmt.Sprintf("no se pudo leer el id: %s", err.Error()),
		})
		return
	}
	...
	var foundedTransaction transaction

	for _, currentTransaction := range *transactions {
		if currentTransaction.Id == idValue {
			foundedTransaction = currentTransaction
			break
		}
	}

	if foundedTransaction.Id == 0 {
		c.JSON(404, gin.H{
			"error": fmt.Sprintf("no se encontró alguna transacción con el id %v", id),
		})
		return
	}

	c.JSON(200, foundedTransaction)
}
